...
class CursoModel
{
    ...
    public function updateInscrito($idInscrito, $pago, $llamado, $comentario){
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $sql = $pdo->prepare("UPDATE inscritos set pagado =:pagado, llamado =:llamado, comentario =:comentario WHERE id=:id");

        if ($sql->execute(array('pagado' => $pago, 'llamado' => $llamado, 'comentario' => $comentario, 'id' => $idInscrito ))) {
            $status  = "success";
